...
use App\Notifications\CandidateValide;
use App\Notifications\CandidateConfirm;
use Illuminate\Support\Facades\Storage;
use App\Notifications\CandidateNotValide;
use Jantinnerezo\LivewireAlert\LivewireAlert;

class CandidatesLatest extends Component
{ 
    ...
    {
        $valide = Candidate::find($this->candidateId);
        if ($valide) {
            $valide->update(['status' => 'Valide']);
            // Mise à jour de l'état local du composant
            $this->status = $valide->status;

            $valide->notify(new CandidateValide()); // Envoi de la notification
            // Affichage d'un message de confirmation
            $this->showMessage('Candidature validée');
    ...
    public function render()
    {
        ...
        $latest_candidates = Candidate::where('status', 'Default')->count();


        return view('livewire.candidates-latest', [
